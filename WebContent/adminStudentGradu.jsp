...
if(session.getAttribute("userID")!=null){             //세션 userID
      ...
      if(request.getParameter("getGradu")!=null){             //검색으로 받아오는 학과
         ...
         <li class="nav-item dropdown"><!-- active는 현재 위치한 페이지에 넣으면 된다./현재 위치가 파란색으로 표시된다. -->
          <a class="nav-link dropdown-toggle active" id="dropdown" data-toggle="dropdown">
           ...
           <div class="dropdown-menu" aria-labelledby="dropdown">
              ...
              <a class="dropdown-item active" href="adminStudentGradu.jsp">졸업조건 미충족 학생 관리</a>
...
%>
   ...
   <section class="container"><!-- 반응형; 특정한 요소가 알아서 작아지게 하는데 도움을 준다. -->
      ...
      <form method="get" action="./adminStudentGradu.jsp" class="form-inline mt-3">
         ...
         <select name="getGradu" class="form-control mx-1 mt-2">
            ...
            <option value="전공 미충족">전공 미충족</option>
            <option value="교양 미충족">교양 미충족</option>
            <option value="종합 미충족">종합 미충족</option>
         </select>
         ...
         <table class="table mx-2 mt-4" style="text-align: center; border: 1px solid #dddddd">
            <thead>
               <tr>
                  ...
                  <th style="background-color:#eeeeee; text-align: center;">미충족 졸업요건</th>
            ...
            <%
               ...
               userList=new UserDAO().getListUser4(search, pageNumber, userDepart, getGradu);
               ArrayList<UserDTO> userList18=new ArrayList<UserDTO>();
               userList18=new UserDAO().getListUser18(search, pageNumber, userDepart, getGradu); //18학번 아래 수정, 핵심역량 관련부분 수정
           	
               if(user.getUserID().indexOf("2018")==-1){
               //System.out.print(getGradu);
               if(userList != null)
                  for(int i=0; i < userList.size(); i++){
                     user = userList.get(i);
                        if(user.isCultureNeceSatisfy()==true && user.isCultureReqSatisfy()==true && (user.isMajorNeceSatisfy()==false || user.isMajorReqSatisfy()==false) && user.isGraduPossible()==false){
                           ab="전공 미충족";
                        }else if((user.isCultureNeceSatisfy()==false || user.isCultureReqSatisfy()==false) && user.isMajorNeceSatisfy()==true && user.isMajorReqSatisfy()==true && user.isGraduPossible()==false){
                           ab="교양 미충족";
                        }else if(((user.isCultureNeceSatisfy()==false || user.isCultureReqSatisfy()==false) &&( user.isMajorNeceSatisfy()==false || user.isMajorReqSatisfy()==false) && user.isGraduPossible()==false)
                        		|| ((user.isCultureNeceSatisfy()==true &&  user.isCultureReqSatisfy()==true) &&( user.isMajorNeceSatisfy()==true && user.isMajorReqSatisfy()==true) && user.isGraduPossible()==false)){
                           ab="종합 미충족";
                        }
            %>
               <tr>
            ...
            <%         
               ...
               if(userList18 != null)
                  for(int i=0; i < userList18.size(); i++){
                     user = userList18.get(i);
                        if(user.isCultureNeceSatisfy()==true && user.isCultureReqSatisfy()==true && (user.isMajorNeceSatisfy()==false || user.isMajorReqSatisfy()==false || user.isFindSatisfy()==false) && user.isGraduPossible()==false){
                           ab="전공 미충족";
                        }else if((user.isCultureNeceSatisfy()==false || user.isCultureReqSatisfy()==false) && user.isFindSatisfy()==true && user.isMajorNeceSatisfy()==true && user.isMajorReqSatisfy()==true && user.isGraduPossible()==false){
                           ab="교양 미충족";
                        }else if(((user.isCultureNeceSatisfy()==false || user.isCultureReqSatisfy()==false) && (user.isMajorNeceSatisfy()==false || user.isMajorReqSatisfy()==false || user.isFindSatisfy()==false) && user.isGraduPossible()==false)
                    		|| ((user.isCultureNeceSatisfy()==true &&  user.isCultureReqSatisfy()==true) &&( user.isMajorNeceSatisfy()==true && user.isMajorReqSatisfy()==true && user.isFindSatisfy()==true) && user.isGraduPossible()==false)){
                           ab="종합 미충족";
                        }
            %>
               <tr>
...
%>
   <a class="page-link" href="./adminStudentGradu.jsp?search=<%= URLEncoder.encode(search, "UTF-8") %>&pageNumber=<%= pageNumber -1 %>">이전</a>
<%
   }
%>
      </li>   
...
%>
   <a class="page-link" href="./adminStudentGradu.jsp?search=<%= URLEncoder.encode(search, "UTF-8") %>&pageNumber=<%= pageNumber +1 %>">다음</a>
<%
   }
%>
      </li>
