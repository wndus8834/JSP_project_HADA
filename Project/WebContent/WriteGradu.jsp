   ...
   <section class="container"><!-- 반응형; 특정한 요소가 알아서 작아지게 하는데 도움을 준다. -->
      <form method="get" action="./WriteGradu.jsp" class="form-inline mt-3"> 
		<select name="userDepart" class="form-control mx-1 mt-2">
         ...
         <input type="text" name="search" class="form-control mx-1 mt-2" placeholder="과목명을 입력해주세요.">
...
%>
		...
		<li>
<%
	if(list.size()<10) { //강의평가가 6개 이상 존재한다면 다음 페이지가 존재한다는 것이기 때문에(한 페이지에 5개까지 보임) 
%>
	  ...
	  <div class="modal-dialog">
		...
		<form  action="./departrUpdate" method="post">
			<div class="form-group">
				...
				<div class="form-group">
					<label>강의명</label>
					<input type="text" name="subjname" id="subjname"class="form-control"readonly>
				</div>
			</div>
			
			<div class="form-group">
				<label>과목코드</label>
				<input type="text" name="subjcode" id="subjcode" class="form-control" readonly>
			</div>
			<div class="form-group">
						<label>학과</label>
						<select name="userdepart" id="userdepart" class="form-control">
