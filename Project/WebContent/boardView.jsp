...
<%
	...
	try{
		...
		PrintWriter script=response.getWriter();
	      ...
	      script.println("alert('페이지 번호가 잘못되었습니다.');");
...
%>
	...
	<div class="row">
		<table class="table table-hover mx-2 mt-4" style="text-align: center; border: 1px solid #dddddd">
			<thead>
			<tr>
				<th style="background-color:#eeeeee; text-align: center;width: 90px;"><h5>번호</h5></th>
				<th style="background-color:#eeeeee; text-align: left;"><h5>제목</h5></th>
				<th style="background-color:#eeeeee; text-align: center;width: 100px;"><h5>작성자</h5></th>
				<th style="background-color:#eeeeee; text-align: center;width: 120px;"><h5>작정 날짜</h5></th>
				<th style="background-color:#eeeeee; text-align: center;width: 80px;"><h5>조회수</h5></th>
			...
			<%
				for(int i=0; i<boardList.size(); i++){
					...
					adminChecked=new UserDAO().getAdminChecked(board.getUserID());
					if(adminChecked==true && (board.getBoardTitle()).indexOf("공지")!=-1){
			%>
			<tr>
				<td><%=board.getBoardID() %></td>
				<td style="text-align: left;">
				<a class="text-dark" href="boardShow.jsp?boardID=<%=board.getBoardID() %>">
			<%
				for(int j =0; j < board.getBoardLevel(); j++){
			%>
				<span class="glyphicon glyphicon-arrow-right" aria-hidden="true" ><red>re:</red></span>
			<%
				}
			
				if(board.getBoardAvailable()==0){ %>
					
					<gray>(삭제된 게시물입니다.)</gray>
				<% 	
				}else{ %>
					<%=board.getBoardTitle() %>
				<% }
			...
			<%if(userID!=null){ %>
			<div class="col-12"><a href="boardWrite.jsp" class="btn btn-primary float-right">글쓰기</a></div>
			<%} %>
			
					<ul class="pagination justify-content-center mt-3" style="margin: 0 auto;">
				<%
					...
					if(startPage != 1){
				%>
					<li><a class="page-link" href="boardView.jsp?pageNumber=<%= startPage -1%>" >이전</a></li>
				
				<%
					} else{
				%>
					<li><a class="page-link" style="color: gray;">이전</a></li>
				<%
					}
					for(int i = startPage; i < Integer.parseInt(pageNumber); i++){
				%>
					<li class="page-item"><a class="page-link" href="boardView.jsp?pageNumber=<%= i%>" ><%=i %></a></li>
				<% 
					}
				%>
					<li class="page-item active"><a class="page-link" href="boardView.jsp?pageNumber=<%= pageNumber%>" ><%= pageNumber %></a></li>
				<% 
				...
				%>	
					<li><a class="page-link" href="boardView.jsp?pageNumber=<%= startPage + 10 %>" >다음</a></li>
				<% 
					} else{
				%>
					<li><a class="page-link" style="color: gray;">다음</a></li>
				<% 		
					}
				%>
